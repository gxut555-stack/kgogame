...
func (this *IniParser) LoadBytes(bt []byte) error {
	conf, err := ini.Load(bt)
	if err != nil {
		this.conf_reader = nil
		return err
	}
...
func (this *IniParser) Reload() error {
	if this.conf_reader == nil {
		return errors.New("conf_reader is nil")
	} else {
		return this.conf_reader.Reload()
	}
}
